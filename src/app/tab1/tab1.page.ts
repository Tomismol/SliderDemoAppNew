...
export class Tab1Page implements OnInit {
  ...
  for (let i = 0; i < myPlaces.length; i++) {
    if (myName === myPlaces[i].Name) { this.myZone = Number(myPlaces[i].Zone); }
}
  ...
  */
  ...
  source() { // code for setting moon or sun image based on time
    ...
    if (hours > 7 && hours < 19) {
     ...
     var src = '../assets/imgs/decorative_sun.png';
    } else {
      src = '../assets/imgs/Full_Moon.png';
    }
 ...
 // if (hours < 8) serc=".."
  ...
  // get current time with time zone adjustments
  timeZone() {
    const myTime = this.currentTime(this.myZone + this.selectedZone);
    return myTime;
  }
